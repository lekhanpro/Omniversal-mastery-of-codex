...
const Home: React.FC = () => {
  ...
  return (
    <div className="p-6 md:p-12 max-w-7xl mx-auto relative z-10">
      ...
      >
        <h1 className="text-4xl md:text-6xl font-black text-gray-900 mb-4 font-mono tracking-tighter">
        ...
        <div className="mt-8 relative max-w-md mx-auto">
          ...
          <input
            ...
            className="block w-full pl-10 pr-3 py-3 bg-white border border-gray-300 rounded-full text-gray-900 placeholder-gray-500 focus:outline-none focus:border-neon-blue focus:ring-1 focus:ring-neon-blue transition-all shadow-sm"
            value={search}
          ...
          {filteredDomains.map((domain, index) => (
            ...
            >
              <Link 
                ...
                className={`block h-full p-6 rounded-xl border border-gray-200 bg-white shadow-sm group relative overflow-hidden ${
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                   <div className={`p-3 rounded-lg ${domain.isLocked ? 'bg-gray-200 text-gray-500' : 'bg-neon-blue/10 text-neon-blue'}`}>
                      {getIcon(domain.icon)}
                   </div>
                   <span className="font-mono text-xs text-gray-400">D-{String(domain.id).padStart(2, '0')}</span>
                </div>
                
                <h2 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-neon-blue transition-colors">
                  {domain.title}
                </h2>
                ...
                {domain.isLocked && (
                    <div className="absolute inset-0 flex items-center justify-center bg-white/60 backdrop-blur-[1px]">
                        <div className="px-3 py-1 border border-gray-300 bg-white rounded text-xs font-mono text-gray-500 uppercase">
                            Encrypted
                        </div>
                    </div>
                )}
